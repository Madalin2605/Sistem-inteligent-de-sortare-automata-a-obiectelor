...
from Interfaces.robot_interface.robots.UR5.urx import urrtmon, ursecmon
...
class URRobot(object):
    ...
    def _wait_for_move(self, target, threshold=None, timeout=10, joints=False):
        ...
        yaml_path = 'C:/Users/campus/Documents/robotsnet_research/UI/web-hmi/src/assets/data_2.yaml'
        ...
        self.logger.debug("Set status_message to 'Clear' in YAML file")
        ...
        while True:
            if not self.is_running():
                data['status_message'] = 'Robot stopped'
                with open(yaml_path, 'w') as yaml_file:
                    yaml.dump(data, yaml_file)
                self.logger.debug("Set status_message to 'Robot stopped' in YAML file")
            ...
            if not self.secmon.is_program_running():
                ...
                count += 1
                if count > timeout * 10:
                    data['status_message'] = 'Goal not reached but no program has been running for seconds'
                    with open(yaml_path, 'w') as yaml_file:
                        yaml.dump(data, yaml_file)
                    self.logger.debug("Set status_message to 'Goal not reached but no program has been running for seconds' in YAML file")
